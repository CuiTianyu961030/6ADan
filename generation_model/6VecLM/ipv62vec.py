...
dataset_path = "generation_model/6VecLM/data/processed_data/word_data.txt"
model_path = 'generation_model/6VecLM/models/ipv62vec.model'
...
def train():
    ...
    model = word2vec.Word2Vec(sentences, alpha=0.025, min_count=1, size=100, window=5,
...
def word_tsne_picture():
    ...
    fig.tight_layout()
    plt.savefig("generation_model/6VecLM/images/word_tsne_ipv62vec_without_text.png", dpi=800)
    ...
    fig.tight_layout()
    plt.savefig("generation_model/6VecLM/images/word_tsne_ipv62vec.png", dpi=800)
    ...
    for i in range(len(X_tsne)):
        ...
        y = X_tsne[i][1]
        plt.text(x, y, vocab[i], size=3)
    plt.savefig("generation_model/6VecLM/images/word_tsne_ipv62vec_content.png", dpi=300)
    ...
    for i in range(len(X_tsne)):
        ...
        y = X_tsne[i][1]
        plt.text(x, y, vocab[i], size=3)
    plt.savefig("generation_model/6VecLM/images/word_tsne_ipv62vec_situation.png", dpi=300)
...
def address_tsne_picture():
    ...
    plt.figure(figsize=(10, 8))
    plt.scatter(x, y, s=5)
    plt.savefig("generation_model/6VecLM/images/address_tsne_ipv62vec.png", dpi=300)
...
def cluster(data):
    ...
    plt.savefig("generation_model/6VecLM/images/cluster_ipv62vec.png", dpi=300)


if __name__ == "__main__":
    print("开始执行 ipv62vec.py ...")
    train()
    # word_tsne_picture()
