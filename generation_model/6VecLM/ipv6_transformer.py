...
word2vec_model_path = 'generation_model/6VecLM/models/ipv62vec.model'
data_path = "generation_model/6VecLM/data/processed_data/word_data.txt"
model_path = "generation_model/6VecLM/models/ipv6_transformer_s6_e10_t0015.model"
generation_path = "generation_model/6VecLM/data/generation_data/candidate_s6_e10_t0015.txt"
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        true_dict = [self.word2vec_model.wv[id2word(int(id.numpy()), self.word2vec_model)] for id in target]
        # print(x.shape)
        # print(x)
...
def next_generation(word2vec_model, vector, temperature, index):
    ...
    similarity = [torch.cosine_similarity(torch.Tensor(index_word_vectors[i]), vector) for i in range(len(index_words))]
...
if __name__ == "__main__":
    print("开始执行 ipv6_transformer.py ...")
    ...
    for i in range(len(test_data)):
        ...
        predict = greedy_decode(model, word2vec_model, src, src_mask,
                                max_len=32-encoder_input_length, start_symbol=start_symbles[i]).numpy()
