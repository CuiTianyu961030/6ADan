...
class GraphConvolution(Layer):
    ...
    def _call(self, inputs):
        ...
        outputs = tf.gather(self.act(x), axis=0, indices=self.node_ids)
        return outputs
...
def get_model(model_str, placeholders, num_features, num_nodes, features_nonzero, client_list):
    model = None
    if model_str == 'gcn_ae':
        model = GCNModelAE(placeholders, num_features, features_nonzero, client_list)

    elif model_str == 'gcn_vae':
        model = GCNModelVAE(placeholders, num_features, num_nodes, features_nonzero, client_list)
...
if __name__ == '__main__':
    ...
    ae_model = get_model(model_str, placeholders, feas['num_features'], feas['num_nodes'],
                                                feas['features_nonzero'], feas['client_list'])

    opt = get_optimizer(model_str, ae_model, placeholders, feas['pos_weight'], feas['norm'], feas['num_nodes'])
