...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        user_inference = {0: [0]}
        address_inference = {0: [converse_ip(address[0])]}
        history_max_id = 0
        margin = 3.5
        ...
        for i in range(1, nb_graph):
            ...
            if distance[closet_node] >= margin:
                ...
                address_inference[history_max_id] = [converse_ip(address[i])]
                pred = history_max_id
            else:
                for user_id, node_id in user_inference.items():
                    if closet_node in node_id:
                        user_inference[user_id].append(i)
                        address_inference[user_id].append(converse_ip(address[i]))
                        pred = user_id
                        break
            ...
            # print('address_list', address_inference.items())
