...
checkpt_file = 'linkage_model/SiamHAN/pre_trained/siamhan-pretrain.ckpt'
checkpt_meta_file = 'linkage_model/SiamHAN/pre_trained/siamhan-pretrain.ckpt.meta'
    ...
    with tf.Session() as sess:
        ...
        for tracking_nb in [100, 200, 300]:
            ...
            for i in range(len(tracking_label)):
                nb_true = 0
                track_ip = converse_ip(tracking_address[i])
                for j in range(nb_graph):
                    if j != index[i]:
                        ...
                        if track_ip not in res_dict.keys():
                            res_dict[track_ip] = []
                        if distance < margin:
                            target_ip = converse_ip(address[j])
                            res_dict[track_ip].append(target_ip)

                nb_total_true += nb_true
                print("search ip: " + track_ip)
                print("linked ip: ", res_dict[track_ip])
